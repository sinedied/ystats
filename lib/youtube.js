...
export async function getStatsForVideos(videoIdsOrUrls, token) {
  ...
  const ids = (Array.isArray(videoIdsOrUrls) ? videoIdsOrUrls : [videoIdsOrUrls])
    .filter(id => id)
    .map(parseId)
    .join(',');
  const stats = [];
...
export async function getStatsForPlaylist(playlistIdOrUrl, token) {
  ...
  try {
    ...
    for (let v of videos) {
      playlistStats.views += v.stats.views;
      playlistStats.likes += v.stats.likes;
      playlistStats.dislikes += v.stats.dislikes;
      playlistStats.favorites += v.stats.favorites;
      playlistStats.comments += v.stats.comments;
    }
    stats.push({
      id: playlistId,
